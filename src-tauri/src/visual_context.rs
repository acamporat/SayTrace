...
/// best-effort at the job-coordinator call site: a visual-source or local-vision
/// failure must never invalidate an otherwise canonical audio transcript.
pub fn enrich_meeting(core: &CoreService, meeting_id: &str) -> CoreResult<VisualContextReport> {
    let result = enrich_meeting_inner(core, meeting_id);
...
/// `visual_context_runs`.
pub fn backfill_next_missing(core: &CoreService) -> CoreResult<Option<VisualContextBackfill>> {
    let Some(meeting_id) = claim_next_missing(core)? else {
        return Ok(None);
    };
    let report = enrich_meeting(core, &meeting_id)?;
...
fn claim_next_missing(core: &CoreService) -> CoreResult<Option<String>> {
    ...
    let inserted = transaction.execute(
        "INSERT OR IGNORE INTO visual_context_runs(
            meeting_id,screen_asset_id,analyzer_version,status,updated_at_ms
         ) VALUES (?1,?2,?3,'running',?4)",
        params![meeting_id, visual_asset_id, ANALYZER_VERSION, now],
    )?;
    transaction.commit()?;
...
fn enrich_meeting_inner(core: &CoreService, meeting_id: &str) -> CoreResult<VisualContextReport> {
    ...
    if config.visual_speaker_attribution {
        match analyze_speakers(core, meeting_id, &visual_path, video_stream_index, &turns) {
...
fn persist_speaker_evidence(
    ...
    connection.execute(
        "INSERT INTO visual_context_events(
         ...
         ) VALUES (?1,?2,?3,'speaker_evidence',?4,?5,'review','local_vision',?6,?7,?8,?9)
         ON CONFLICT(id) DO UPDATE SET
