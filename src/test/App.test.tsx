...
describe("SayTrace workspace", () => {
  ...
  it("starts a browser-preview recording from the new transcription dialog", async () => {
    ...
    await user.click(
      within(confirmation).getByRole("checkbox", {
        name: /I understand that every connected display/i,
      }),
    );
    await user.click(
  ...
  it("defaults screen context on and requires explicit final whole-desktop acknowledgement", async () => {
    const user = userEvent.setup();
    render(<App />);
    ...
    expect(
      within(dialog).getByRole("checkbox", {
        name: /Record all connected displays/i,
      }),
    ).toBeChecked();
    expect(
      within(dialog).getByRole("checkbox", {
    ...
    expect(
      within(dialog).getByText(/Everything visible across all connected displays/i),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText("All connected displays are being recorded"),
    ).toBeInTheDocument();
